  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const config: Record<string, { label: string; className: string }> = {
        failed:     { label: 'Duplicata',    className: 'border-red-900/40 bg-red-950/30 text-red-400' },
        processing: { label: 'Tag ausente',  className: 'border-amber-900/40 bg-amber-950/30 text-amber-400' },
        pending:    { label: 'Inadimplente', className: 'border-orange-900/40 bg-orange-950/30 text-orange-400' },
        paid:       { label: 'Resolvido',    className: 'border-emerald-900/40 bg-emerald-950/30 text-emerald-400' },
      }
      const { label, className } = config[status] ?? { label: status, className: '' }
      return (
        <Badge variant='outline' className={`rounded-full px-2.5 py-0.5 text-[11px] font-normal gap-1.5 ${className}`}>
